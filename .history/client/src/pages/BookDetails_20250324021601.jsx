...
const BookDetails = () => {
    ...
    // 📌 Fetch Sample Pages from Gutenberg API
    const fetchSamplePages = async () => {
        try {
            const response = await fetch(`https://gutendex.com/books?title=${book.title}`);
            const data = await response.json();

            if (data.results.length > 0) {
                const bookData = data.results[0]; // First matching book
                const firstBookId = bookData.id;

                // Fetch book text
                const textResponse = await fetch(`https://www.gutenberg.org/cache/epub/${firstBookId}/pg${firstBookId}.txt`);
                ...
                setSamplePages([
                    ...
                    "The sun set over the horizon as the hero embarked on a journey...",
                    "Deep in the forest, a hidden treasure awaited discovery..."
                ]);
            }
